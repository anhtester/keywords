...
public class WebUI {
    ...
    public static void getToUrlAuthentication(String url, String username, String password) {
        ...
        openWebsite(url);
        //sleep(3); // Wait is now handled by openWebsite -> waitForPageLoaded
    }
    ...
    public static String getCurrentUrl() {
        // smartWait(); // Removed to avoid slow down
    ...
    public static boolean checkElementExist(By by) {
        // Use implicit wait from getWebElements but usually checking existence shouldn't wait too long if not needed
        // But for stability, a small wait is better.
        try {
            WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(FrameworkConstants.WAIT_EXPLICIT));
    ...
    public static boolean checkElementExist(By by, int maxRetries, int waitTimeMillis) {
        int retryCount = 0;
        int timeOutSeconds = (maxRetries * waitTimeMillis) / 1000;
    ...
    public static boolean isElementDisplayed(By by, int second) {
        // Renamed logic: Use WebDriverWait instead of loop + sleep
        try {
    ...
    public static void clickElement(By by) {
        // smartWait(); // Removing global wait to improve speed and rely on explicit wait below

        // Add robust retry logic for StaleElementReferenceException
        int maxRetry = 3;
        for (int i = 0; i < maxRetry; i++) {
            try {
                ...
                LogUtils.info("Retry click element due to StaleElementReferenceException: " + by);
            } catch (ElementClickInterceptedException e) {
                if (i == maxRetry - 1) throw e;
                LogUtils.info("Retry click element due to ElementClickInterceptedException: " + by);
                // Optional: Scroll to element again or wait slightly
...
//    }
    ...
    public static WebElement waitForElementVisible(By by, int second) {
        try {
            WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(second));
            // Removed redundant isElementVisible check to rely on WebDriverWait efficiency
    ...
    public static WebElement waitForElementVisible(By by) {
        // waitForElementPresent(by); // Removed to avoid double wait. visibilityOfElementLocated checks presence internally.
